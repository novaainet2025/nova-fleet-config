> v2 보정 (2026-07-03, 감독관 nova-macstudio-claude-2 기술검토 반영): macstudio Docker 미설치(T1) → pip venv를 1순위로, macOS 경로(~/.marm/*), ssh root 전제 삭제. 감독관 조건부 승인 상태.
...
#### Option A: pip venv (1순위 — macstudio Docker 미설치 T1 확인, kangNote 파일럿 검증 방식)
```bash
...
marm-mcp-server \  # (미확인) --bind/--port 플래그명 — kangNote 파일럿은 기본 127.0.0.1:8001 + env MARM_API_KEY로 기동함
  --db-path ~/.marm/data/marm.db \
  --api-key "$(openssl rand -hex 32)"
```

#### Option B: Docker (Docker Desktop 신규 설치 사용자 결재 후 — 2순위)
```bash
# nova-macstudio에서 실행
docker run -d \
  --name marm-mcp \
...
- **권장**:
  1. nova-macstudio에서 `~/.marm/.env.marm` 로컬 생성 (chmod 600)
...
```bash
# 매주 일요일 03:00 (cron)
0 3 * * 0 cp ~/.marm/data/marm.db ~/.marm/backup/marm-$(date +\%Y\%m\%d).db
0 3 * * 0 tar -czf ~/.marm~/.marm/backup/marm-config-$(date +\%Y\%m\%d).tar.gz /etc/marm/
```
...
```bash
# 1. nova-macstudio 준비 (해당 머신 세션에서 직접 실행 — ssh/root 불필요)
mkdir -p ~/.marm/data ~/.marm/backup

# 2. Docker 배포
MARM_KEY=$(openssl rand -hex 32)
echo "MARM_API_KEY=$MARM_KEY" > ~/.marm/.env.marm
