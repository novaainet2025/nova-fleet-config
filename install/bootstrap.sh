...
# ══════════════════════════════════════════════════════════════════════════
# [16.5] fleet-sync.sh cron 자동 등록 (1시간 간격 자동 동기화)
# ══════════════════════════════════════════════════════════════════════════
FLEET_SYNC="$FLEET_DIR/install/fleet-sync.sh"
FLEET_LOG="$HOME/.claude/logs/fleet-sync.log"
mkdir -p "$HOME/.claude/logs"
chmod +x "$FLEET_SYNC" 2>/dev/null || true

if [[ -f "$FLEET_SYNC" ]]; then
  # crontab에 이미 등록됐는지 확인 (멱등)
  CRON_ENTRY="7 * * * * bash $FLEET_SYNC >> $FLEET_LOG 2>&1"
  if crontab -l 2>/dev/null | grep -qF "fleet-sync.sh"; then
    ok "fleet-sync cron 이미 등록됨"
  else
    # 기존 crontab + 새 항목 추가
    (crontab -l 2>/dev/null; echo "$CRON_ENTRY") | crontab - \
      && ok "fleet-sync cron 등록 완료 (매 시간 7분)" \
      || warn "cron 등록 실패 — 수동: crontab -e 후 '$CRON_ENTRY' 추가"
  fi
else
  warn "fleet-sync.sh 없음 — nova-fleet-config 업데이트 후 재실행"
fi
