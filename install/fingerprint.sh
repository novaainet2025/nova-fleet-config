...
echo "providers: $(for p in claude codex gemini cursor-agent copilot opencode vllm higgsfield gbrain; do { command -v $p >/dev/null 2>&1 || [ -x "$HOME/.bun/bin/$p" ]; } && printf '%s ' $p; done)"
