...
1. **모든 작업 보고**에 `## 검증 영수증` 포함 — `[변경] [검증방법] [등급] [Gap] [미검증항목] [자동재진행]`.
...
10. **자동 재진행 상태 필수** — `[자동재진행] done | continue | blocked: <이유>` 중 하나를 기록. `done`은 Gap 100% + 미검증항목 없음일 때만 허용.
...
## 종료 불변식 — 남은 작업 자동 점검·재진행
1. 실질적 작업 응답을 끝내기 전에 원 요청 범위, 활성 plan/checklist, 테스트·검증 실패, 위임 결과, `[미검증항목]`을 다시 대조한다.
2. 현재 권한 안에서 실행 가능한 작업이 하나라도 남으면 종료하지 않는다. 영수증에 구체적인 잔여 항목과 `[자동재진행] continue`를 기록하고 즉시 계속한다.
3. 서로 독립적인 잔여 작업은 NCO `conductor`/`parallel` 한 배치로 병렬 실행한다. 프로바이더 부족 시 직접 병렬 검증으로 폴백하고 사유를 영수증에 남긴다.
4. Stop 훅은 최신 영수증이 `Gap < 100%`, 미검증·남은 작업 존재, 또는 `continue`이면 자동 재진입한다. `Gap 100%` + `[미검증항목] 없음` + `done`일 때만 정상 종료한다.
5. 사용자 결정·승인, 자격 증명·권한, 외부 상태 변경이 꼭 필요하면 `[자동재진행] blocked: <정확한 이유>`로 기록하고 멈춘다. 자동 재진행이 작업 권한을 넓히지는 않는다.
6. 현재 작업과 무관한 기존 dirty worktree는 그 자체로 잔여 작업이 아니다. 훅의 총 반복 상한(기본 10회)과 stale-transcript 재검사는 runaway 방지용으로 유지한다.
