...
install/
  ...
  sync-on-connect.sh # SessionStart 자동 드리프트 점검(report-only·비차단·apply opt-in)
VERSION             # 정본 버전(= git short sha 로도 대체 가능)
```

## Quick start — 새 노드 설치 (1클릭 동기화)
```bash
# ★ repo는 반드시 $HOME/nova-fleet-config 에 클론 (sync-on-connect 훅이 이 경로 참조)
git clone <this-repo-url> "$HOME/nova-fleet-config"
cd "$HOME/nova-fleet-config"
bash install/apply.sh --dry-run        # 먼저 미리보기
bash install/apply.sh --merge-settings # 적용(파일sync + settings UNION 머지). 백업 자동.
bash install/sync-on-connect.sh        # "✓ canonical 동기화됨" 확인
```
- 적용 후 **세션 시작마다** SessionStart 훅이 `sync-on-connect.sh`를 돌려 드리프트를 자동 리포트(비차단·report-only). 자동 적용을 원하면 `NCO_FLEET_AUTOSYNC=1`.
- 롤백: `~/.claude/_fleet-backup-*` / `settings.json.fleet-bak`.
